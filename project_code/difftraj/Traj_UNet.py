...
class WideAndDeep(nn.Module):
    def __init__(self, embedding_dim=128, hidden_dim=256, cat_attrs:list[int]=[0,6,7,8]):
        ...
        self.deep_fc1 = nn.Linear(hidden_dim*len(self.cat_attrs), embedding_dim)
    ...
    def forward(self, attr):
        ...
        depature, sid, eid, has_FuelRate = [attr[:, i].long() for i in self.cat_attrs]

        # Wide part
        wide_out = self.wide_fc(continuous_attrs)

        # Deep part
        embeddings = [self.depature_embedding(item) for item in [depature, sid, eid, has_FuelRate]]
